...
class K8sTestEnvironment:
    ...
    def deploy_application(self):
        ...
        # Fixed: waiting on pods directly races against Pod creation
        # (the Deployment controller needs a moment to spawn them after
        # `apply`), which can fail with "no matching resources found"
        # even though the deployment succeeds moments later. Waiting on
        # rollout status avoids that race.
        logger.info("Waiting for deployments to be ready...")
        self.run_command(
            "kubectl rollout status deployment/dev-backend -n study-app --timeout=120s"
        )
        self.run_command(
            "kubectl rollout status deployment/dev-frontend -n study-app --timeout=120s"
        )
